@extends('layouts.app')
@section('title', 'Import Data Diri mahasiswa')

@push('head')
<style>
  /* ── Custom styles untuk halaman import mahasiswa ── */
  .step-connector {
    position: absolute;
    top: 20px;
    left: calc(50% + 24px);
  ...
  .mahasiswa-card:focus-within {
    box-shadow: 0 0 0 3px rgba(59,130,246,.12);
    border-color: #93c5fd;
  }
  .mahasiswa-card.disabled {
    opacity: .45;
    pointer-events: none;
  }
...
<div class="flex flex-col sm:flex-row sm:items-start justify-between gap-4 mb-6">
  <div>
    <div class="flex items-center gap-2 text-sm text-gray-400 mb-2">
      ...
      <span class="text-gray-700 font-medium">Import Data Diri mahasiswa</span>
    </div>
    <h1 class="text-2xl font-bold text-gray-800 flex items-center gap-2">
      <span class="bg-blue-100 text-blue-600 rounded-xl p-2 text-base">
        <i class="fa-solid fa-chalkboard-user"></i>
      </span>
      Import Data Diri mahasiswa
    </h1>
    <p class="text-gray-400 text-sm mt-1">
      Tambahkan mahasiswa baru ke sistem — satu per satu atau sekaligus lewat Excel.
    </p>
  </div>
...
<div id="panel-manual">
  ...
  <form method="POST" action="{{ route('admin.mahasiswa-import.simpan') }}" id="formManual" novalidate>
    @csrf
    ...
    <div id="mahasiswaList" class="space-y-4 mb-5">
      {{-- Kartu pertama selalu ada --}}
      <div class="mahasiswa-card bg-white rounded-2xl border border-gray-200 p-5 anim-in" id="card-0" data-index="0">
        @include('admin.mahasiswa._card', ['i' => 0, 'nama' => old('mahasiswa.0.nama', ''), 'email' => old('mahasiswa.0.email',''), 'nip' => old('mahasiswa.0.nip',''), 'prodi' => old('mahasiswa.0.program_studi','')])
      </div>
    </div>
    ...
    <div class="bg-amber-50 border border-amber-200 rounded-2xl p-4 flex items-start gap-3 mb-6">
      ...
      <div class="text-sm">
        <p class="font-semibold text-amber-800 mb-0.5">Password default: <code class="bg-amber-100 px-1.5 py-0.5 rounded font-mono text-xs">mahasiswa123</code></p>
        <p class="text-amber-700 text-xs">mahasiswa dapat menggantinya setelah login pertama. Pastikan NIP/NIM atau email dikomunikasikan ke mahasiswa yang bersangkutan.</p>
      </div>
    </div>
...
<div id="panel-csv" class="hidden">
  ...
  <div class="bg-white rounded-2xl border border-gray-200 p-5 mb-4">
    <h3 class="font-semibold text-gray-700 mb-1 flex items-center gap-2">
      <i class="fa-solid fa-paste text-indigo-500"></i>Paste Data mahasiswa
    </h3>
...
<template id="cardTemplate">
  <div class="mahasiswa-card bg-white rounded-2xl border border-gray-200 p-5 anim-in" id="card-__IDX__" data-index="__IDX__">
    @include('admin.mahasiswa._card', ['i' => '__IDX__', 'nama' => '', 'email' => '', 'nip' => '', 'prodi' => ''])
  </div>
</template>
...
function tambahKartu(data = {}) {
  ...
  document.getElementById('mahasiswaList').appendChild(card);

  // Isi nilai jika ada data dari CSV
  if (data.nama)  card.querySelector(`[name="mahasiswa[${idx}][nama]"]`).value  = data.nama;
  if (data.email) card.querySelector(`[name="mahasiswa[${idx}][email]"]`).value = data.email;
  if (data.nip)   card.querySelector(`[name="mahasiswa[${idx}][nip]"]`).value   = data.nip;
  if (data.prodi) card.querySelector(`[name="mahasiswa[${idx}][program_studi]"]`).value = data.prodi;
...
function hapusKartu(idx) {
  ...
  if (!card) return;
  // Minimal 1 kartu
  const semua = document.querySelectorAll('.mahasiswa-card');
  if (semua.length <= 1) {
...
function updateJumlah() {
  const n = document.querySelectorAll('.mahasiswa-card').length;
  const el = document.getElementById('jumlahLabel');
  if (el) el.textContent = n + ' mahasiswa';
}
...
function konfirmasiCsv() {
  ...
  const d0 = csvData[0];
  totalKartu = 1;
  const first = `<div class="mahasiswa-card bg-white rounded-2xl border border-gray-200 p-5 anim-in" id="card-0" data-index="0">
    <div class="flex items-start justify-between mb-4">
      <div class="flex items-center gap-2">
        <span class="bg-blue-100 text-blue-600 text-xs font-bold px-2.5 py-1 rounded-lg">mahasiswa #1</span>
      </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
      <div class="sm:col-span-2">
        ...
        <input type="text" name="mahasiswa[0][nama]" value="${esc(d0.nama)}" placeholder="Nama lengkap dengan gelar" required
      ...
      <div>
        ...
        <input type="email" name="mahasiswa[0][email]" value="${esc(d0.email)}" placeholder="email@kampus.ac.id" required
      ...
      <div>
        ...
        <input type="text" name="mahasiswa[0][nip]" value="${esc(d0.nip)}" placeholder="Nomor Induk Pegawai"
          class="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
      </div>
    </div>
    <div>
      <label class="block text-xs font-semibold text-gray-500 mb-1.5 uppercase tracking-wide">Program Studi</label>
      <input type="text" name="mahasiswa[0][program_studi]" value="${esc(d0.prodi)}" placeholder="Teknik Informatika"
        class="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
    </div>
  </div>`;
  document.getElementById('mahasiswaList').innerHTML = first;
