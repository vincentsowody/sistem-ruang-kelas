...
@section('page_title', 'Tambah Ruang Kelas')
@section('page_subtitle', 'Daftarkan ruang kelas atau laboratorium baru')
...
<div class="mb-5">
    <div class="flex items-center gap-2 text-sm text-slate-400 mb-3">
        ...
        <span class="text-slate-800 font-medium">Tambah Ruang</span>
    </div>
    <h1 class="text-lg font-bold text-slate-800">Tambah Ruang Kelas</h1>
</div>
    ...
    <form method="POST" action="{{ route('admin.ruang.store') }}" class="space-y-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="kode_ruang" value="{{ old('kode_ruang') }}"
                        ...
                        class="w-full border rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent {{ $errors->has('kode_ruang') ? 'border-red-400 bg-red-50' : 'border-slate-200' }}">
                    @error('kode_ruang')
                ...
                <div>
                    ...
                    <input type="text" name="nama_ruang" value="{{ old('nama_ruang') }}"
                        ...
                        class="w-full border rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent {{ $errors->has('nama_ruang') ? 'border-red-400 bg-red-50' : 'border-slate-200' }}">
                    @error('nama_ruang')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="gedung" value="{{ old('gedung') }}"
                        ...
                        list="gedung-list"
                        class="w-full border rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent {{ $errors->has('gedung') ? 'border-red-400 bg-red-50' : 'border-slate-200' }}">
                ...
                <div>
                    ...
                    <input type="number" name="lantai" value="{{ old('lantai', 1) }}" min="1" max="20"
                        class="w-full border rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent {{ $errors->has('lantai') ? 'border-red-400 bg-red-50' : 'border-slate-200' }}">
                    @error('lantai')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="number" name="kapasitas" value="{{ old('kapasitas') }}" min="1"
                        ...
                        class="w-full border rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent {{ $errors->has('kapasitas') ? 'border-red-400 bg-red-50' : 'border-slate-200' }}">
                    @error('kapasitas')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                <div>
                    ...
                    <select name="jenis"
                        class="w-full border rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-white {{ $errors->has('jenis') ? 'border-red-400' : 'border-slate-200' }}">
                ...
                <div class="flex gap-4">
                    ...
                    <label class="flex items-center gap-2 cursor-pointer">
                        <input type="radio" name="status" value="{{ $s }}"
                            ...
                            class="text-blue-600 focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                        <span class="text-sm text-slate-700 capitalize">{{ $s }}</span>
                    </label>
                    @endforeach
                </div>
            </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-6">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 gap-3">
                ...
                <label class="flex items-center gap-3 p-3 border rounded-xl cursor-pointer hover:bg-blue-50 hover:border-blue-300 transition
                              {{ in_array($value, $oldFasilitas) ? 'bg-blue-50 border-blue-300' : 'border-slate-200' }}">
                    <input type="checkbox" name="fasilitas[]" value="{{ $value }}"
                        {{ in_array($value, $oldFasilitas) ? 'checked' : '' }}
                        class="text-blue-600 rounded focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                    <i class="fa-solid {{ $opt['icon'] }} text-slate-400 text-sm w-4"></i>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-6">
            ...
            <textarea name="keterangan" rows="3" placeholder="Catatan tambahan tentang ruang ini..."
                class="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent resize-none">{{ old('keterangan') }}</textarea>
        </div>
