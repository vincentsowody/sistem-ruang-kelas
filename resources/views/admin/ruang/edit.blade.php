...
<div class="mb-6">
    <div class="flex items-center gap-2 text-sm text-gray-500 mb-2">
        ...
        <span class="text-gray-800 font-medium">Edit {{ $ruang->kode_ruang }}</span>
    </div>
    <h1 class="text-2xl font-bold text-gray-800">Edit Ruang Kelas</h1>
</div>

<div class="max-w-2xl">
    <form method="POST" action="{{ route('admin.ruang.update', $ruang) }}" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 space-y-5">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="kode_ruang" value="{{ old('kode_ruang', $ruang->kode_ruang) }}"
                    ...
                    @error('kode_ruang')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1.5">
                        Nama Ruang <span class="text-red-500">*</span>
                    </label>
                    <input type="text" name="nama_ruang" value="{{ old('nama_ruang', $ruang->nama_ruang) }}"
                    ...
                    @error('nama_ruang')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="gedung" value="{{ old('gedung', $ruang->gedung) }}"
                        list="gedung-list"
                    ...
                    @error('gedung')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1.5">
                        Lantai <span class="text-red-500">*</span>
                    </label>
                    <input type="number" name="lantai" value="{{ old('lantai', $ruang->lantai) }}" min="1" max="20"
                    ...
                    @error('lantai')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="number" name="kapasitas" value="{{ old('kapasitas', $ruang->kapasitas) }}" min="1"
                ...
                <div>
                    ...
                    <select name="jenis"
                        ...
                        <option value="{{ $j }}" {{ old('jenis', $ruang->jenis) == $j ? 'selected' : '' }}>{{ ucfirst($j) }}</option>
                        @endforeach
                    </select>
                ...
                <div class="flex gap-4">
                    ...
                    <label class="flex items-center gap-2 cursor-pointer">
                        <input type="radio" name="status" value="{{ $s }}"
                            {{ old('status', $ruang->status) == $s ? 'checked' : '' }}
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 gap-3">
                @php
                    ...
                    $currentFasilitas = old('fasilitas', $ruang->fasilitas ?? []);
                @endphp
                ...
                <label class="flex items-center gap-3 p-3 border rounded-xl cursor-pointer hover:bg-blue-50 hover:border-blue-300 transition
                              {{ in_array($value, $currentFasilitas) ? 'bg-blue-50 border-blue-300' : 'border-gray-200' }}">
                    <input type="checkbox" name="fasilitas[]" value="{{ $value }}"
                        {{ in_array($value, $currentFasilitas) ? 'checked' : '' }}
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            ...
            <textarea name="keterangan" rows="3"
                class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none">{{ old('keterangan', $ruang->keterangan) }}</textarea>
        </div>
